...
int CAMlink::linkIntersection(double* Qx, double* Qy, double *Rx , double* Ry,
double tol, double& s)
{
//
...
//
    ...
    double rScale0 = sqrt(A[0][0]*A[0][0] + A[0][1]*A[0][1]);
    double rScale1 = sqrt(A[1][0]*A[1][0] + A[1][1]*A[1][1]);
    ...
    rScale0 =  fabs(A[0][0])/rScale0;
    rScale1 =  fabs(A[1][0])/rScale1;
...
int CAMlink::onLink(double xa, double ya, double xb, double yb,
...
//
    ...
    {d = sqrt(xpb*xpb + ypb*ypb); return 1;}

    double xpa = (xp - xa);
    double ypa = (yp - ya);

    if((s <  0.0)&&((xpa*xpa + ypa*ypa) < tol2))
    {d = sqrt(xpa*xpa + ypa*ypa); return 1;}

    return 0;
}


int CAMlink::onConnectedLinks(double* X, double*Y , long nLinks,
              double xp, double yp, double tol, long& linkIndex,
              double& s)
{
//
...
double CAMlink::normalize(double sin)
{
	double s = sin;
	if(s <= 0.0) s = 0.0;
	else if(s >= 1.0) s = 1.0;
